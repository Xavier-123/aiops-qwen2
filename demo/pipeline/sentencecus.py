...
class SentenceSplitter(MetadataAwareTextSplitter):
    ...
    def __init__(
        ...
        secondary_paragraph_chunking_regex: str = CHUNKING_REGEX,
        ...
        print ("Using self define sentence splitter")
        ...
        self._split_fns = [
            ...
            split_by_regex(secondary_paragraph_chunking_regex),
            # No sentence splitter
            #self._chunking_tokenizer_fn,
        ]

        # No sentence split functions
    ...
    def _split(self, text: str, chunk_size: int) -> List[_Split]:
        ...
        text_splits_by_fns, is_sentence, not_splitable = self._get_splits_by_fns(text)

        text_splits = []
        for text_split_by_fns in text_splits_by_fns:
            token_size = self._token_size(text_split_by_fns)
            # Add new part to avoid dead loop
            if token_size <= chunk_size or not_splitable:
                text_splits.append(
                    _Split(
                        text_split_by_fns,
    ...
    def _get_splits_by_fns(self, text: str) -> Tuple[List[str], bool]:
        ...
        for idfunc, split_fn in enumerate(self._sub_sentence_split_fns):
            splits = split_fn(text)
            if len(splits) > 1:
                break
        endfunc = True if idfunc == len(self._sub_sentence_split_fns)-1 else False

        return splits, False, endfunc
